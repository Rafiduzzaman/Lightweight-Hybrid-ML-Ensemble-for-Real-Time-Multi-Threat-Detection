...
\begin{abstract}
Web application firewalls encounter increasingly heterogeneous and dynamic threats, but many models of intrusion detection have been found to be brittle across multiple datasets and distributions of attacks. In this paper, we propose an ensemble based on the lightweight combination of KNN, Random Forest, and XGBoost via weighted soft voting, alongside the use of SMOTE and feature selection, for multi-threat detection in web application firewalls. Our within-dataset experiments reveal good performance on CIC-IDS-2017, UNSW-NB15, and CIC-IDS-2018, where the proposed ensemble achieves 99.56\% accuracy on CIC-IDS-2017, 100.00\% on UNSW-NB15, and 90.42\% on CIC-IDS-2018. An ablation study shows that the proposed ensemble architecture, alongside the feature selection and SMOTE, contribute to our results, while our cross-dataset experiments show that there is significant drop in performance when evaluating our method across different datasets.
\end{abstract}
...
Web application firewalls are supposed to detect a wider variety of malicious patterns while maintaining zero latency and minimal costs during inference. However, achieving this goal is not easy due to the diversity of attacks, the class imbalance problem, and the fact that the classifier with high accuracy on one dataset may suffer from a significant drop in other settings. Therefore, there is a gap between the theoretical performance of machine learning algorithms in the lab and practical use cases in which they have to generalize from a specific dataset to the whole network traffic.
