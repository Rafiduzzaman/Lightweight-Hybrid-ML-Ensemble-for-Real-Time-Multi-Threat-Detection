...
\author{
\IEEEauthorblockN{Md. Fahad Islam Togor, Md. Shakhawat Hossain Khan, Md. Rafiduzzaman, Mahzabeen Nourin, Md. Mazid-ul-Haque}
...
\begin{abstract}
Web Application Firewalls are facing more and more diverse threats, but existing intrusion detection systems lack robustness in transferring across datasets and attacks. In order to tackle this problem, this paper proposes a lightweight hybrid ensemble, which employs a combination of KNN, Random Forest, and XGBoost using weighted soft voting together with SMOTE and feature selection to achieve efficient multi-thread intrusion detection. Experiments conducted inside each dataset illustrate high efficiency on the CICIDS-2017, UNSW-NB15, and CIC-IDS-2018 datasets, achieving 99.56\%, 100.00\%, and 90.42\% accuracy, respectively. The ablation experiments suggest that the hybrid ensemble, feature selection, and SMOTE all contribute to the achieved performance, while tests across datasets show significant performance deterioration under distribution shift. The major contribution of this paper lies in proposing an effective and replicable ensemble pipeline that is efficient despite limited computational resources. This method achieves impressive results when working within one dataset and when transferred to various other intrusion detection benchmarks.
\end{abstract}
...
This paper makes three main contributions. First, we introduce a compact ensemble model for multiple threat detection systems that works well with flow-based WAF data. Second, we conduct a comprehensive evaluation process using three benchmarking datasets, along with an ablation analysis that focuses on how pre-processing, feature selection, and resampling affect the results. Finally, we perform cross-dataset evaluations to analyze the model's sensitivity to changes in the dataset, which reveals valuable information about its robustness.

Another problem is that many intrusion detection algorithms focus on training on a single data set and validating within the same dataset. Nevertheless, the literature on intrusion detection, imbalance resolution, and ensemble learning suggests that high accuracy can be achieved by using appropriate pre-processing steps, balancing classes, and ensuring diversity of models \cite{cover1967nearest,breiman2001random,chen2016xgboost,chawla2002smote,he2009learning}. Many published systems, however, stop at internal validation without measuring their ability to transfer to different environments.
The proposed approach aims to tackle this shortcoming by designing a light-weighted hybrid ensemble suitable for tabular network-flow features. Specifically, the data pipeline involves numeric pre-processing only, followed by normalization, feature selection, and SMOTE oversampling before training an ensemble with a soft voting strategy consisting of KNN, Random Forest and XGBoost. The overall approach ensures simplicity of implementation suitable for real-world application of WAF, while keeping a good detection performance on benchmarks.

We have built a weighted soft voting ensemble with KNN, RF, XGBoost [0.2,0.4,0.4] with SMOTE and feature selection.

What we did: A weighted soft voting ensemble of KNN, Random Forest, and XGBoost [0.2, 0.4, 0.4] was implemented. We used SMOTE oversampling with k=5 and SelectKBest with k=30 for feature selection. The experiments were conducted on 3 benchmark datasets: CIC-IDS-2017 (2.8 million observations), UNSW-NB15 (447 thousand observations), CSE-CIC-IDS2018 (16 million observations reduced to 90 thousand). Seven models served as baseline models.

This paper makes three major contributions. First, we introduce a compact ensemble model that can detect multiple threats in a manner suitable for flow-based WAF telemetry data. Second, we conduct a comprehensive performance evaluation based on three well-known benchmark datasets and perform an ablation analysis to investigate the effect of each component such as pre-processing, feature selection, and resampling techniques. Third, cross-dataset performance tests are conducted to reveal dataset shift sensitivity.

\section{Related Work}
Initial studies on pattern classification highlighted the significance of using nearest-neighbor classifiers for decision rules in supervised learning problems, as well as intrusion detection-like discrimination problems \cite{cover1967nearest}. Ensemble techniques became the go-to approach for enhancing robustness and variance reduction, where bagging and random forest offered solid baselines for tabular data \cite{breiman2001random,polikar2006ensemble,kuncheva2004combining}. Boosting methods further enhanced the quality of predictions in structured datasets by iteratively addressing residual errors and capturing non-linear relationships between features \cite{chen2016xgboost}.

Imbalanced learning studies have direct connections with intrusion detection as attacks are often rare and under-represented in most datasets. The method SMOTE demonstrated the possibility of increasing minority classes via synthetic oversampling in feature space \cite{chawla2002smote}, while future works stressed out the importance of a careful evaluation of cost-sensitive and oversampling algorithms in highly imbalanced class distributions \cite{he2009learning}. It is important to use other metrics beside accuracy when working with such data sets, such as precision, recall, F1-score, and ROC-AUC \cite{sokolova2009systematic}.
...
\subsection{Comparison With Recent Related Works}
Table~\ref{tab:related_comparison} compares the proposed model with recent related hybrid approaches reported in the literature. The proposed weighted ensemble remains competitive across the four common classification metrics and is especially strong on balanced detection quality, as reflected by the F1-score and recall values.

\begin{table*}[htbp]
\caption{Comparison With Recent Related Works.}
\label{tab:related_comparison}
\centering
\small
\begin{tabular}{lcccc}
\toprule
Comparative Entry Name & Accuracy (\%) & Precision & Recall & F1-Score \\
\midrule
Proposed Weighted Ensemble (KNN + RF + XGBoost) & 99.56 & 99.27 & 99.54 & 99.40 \\
SIDNet-XGBoost & 100.00 & 100.00 & 100.00 & 100.00 \\
PhishGuardCore & 97.50 & 97.80 & N/A & N/A \\
CNN-CapsNet-XGBoost-RF Hybrid & 98.00 & 97.30 & 98.80 & 98.00 \\
State Estimation-LightGBM & 98.12 & N/A & 97.61 & 97.16 \\
GPT-Transformer Hybrid & 99.00 & N/A & N/A & N/A \\
\bottomrule
\end{tabular}
\end{table*}
...
We reduced the dimensionality to $k=30$ using SelectKBest to focus the ensemble on the most informative variables. To deal with class imbalance, we used SMOTE with $k=5$ nearest neighbors, which creates synthetic minority samples from the local neighborhood, instead of simply copying observations \cite{chawla2002smote}. The combination of feature selection and resampling was used to reduce overfitting and improve the learnability of the minority class.

\subsection{Ensemble Design}
The proposed classifier is a soft-voting VotingClassifier based on KNN, Random Forest and XGBoost with weights [0.2, 0.4, 0.4]. The weighting scheme captures the relatively stronger empirical contribution of tree-based learners on these tabular datasets, while still preserving the complementary distance-based signal from KNN. Soft voting averages the class probabilities predicted by each individual classifier and is useful when the base learners provide calibrated or approximately calibrated confidences \cite{dietterich2000ensemble,polikar2006ensemble}.

\begin{figure*}[htbp]
\centering
\includegraphics[width=1.3\textwidth, height=5.6in, keepaspectratio]{assets/Untitled Diagram.drawio.pdf}
\caption{Architecture of the proposed lightweight hybrid ensemble for WAF threat detection. Four-stage pipeline: (Stage 1) horizontal data processing flow—raw network flows through preprocessing, feature selection (SelectKBest, k=30), SMOTE balancing (k=5 neighbors), and train-test split (80/20); (Stage 2) parallel training of three diverse learners—KNN (k=5), Random Forest (n=100), and XGBoost (n=50); (Stage 3) weighted soft-voting ensemble with weights [0.2, 0.4, 0.4]; (Stage 4) final binary classification (Normal/Attack) with per-dataset and per-class evaluation metrics.}
...
\bibitem{periasamy2026sidnet}
S. Periasamy, D. G. R. Darshan, K. V. Varnika, G. B. L. Gangothri, and L. P. Teja, ``SIDNet-XGBoost: A hybrid deep learning framework for accurate SQL injection detection,'' in \textit{Proc. 5th Int. Conf. Sentiment Analysis and Deep Learning (ICSADL)}, 2026, doi: 10.1109/ICSADL67539.2026.11451819.

\bibitem{yang2025anomaly}
X. Yang, S. Yuan, L. Pan, X. Wei, and H. Xue, ``Anomaly detection method for smart grids integrating state estimation and LightGBM,'' in \textit{Proc. 5th Int. Conf. Energy, Power and Electrical Engineering (EPEE)}, 2025, doi: 10.1109/EPEE67527.2025.11428577.

\bibitem{ds2025enhancing}
D. S. B, G. M, J. M, and R. Keerthika, ``Enhancing web security using hybrid model,'' in \textit{Proc. 6th Int. Conf. Inventive Research in Computing Applications (ICIRCA)}, 2025, doi: 10.1109/ICIRCA65293.2025.11089651.

\bibitem{aariya2025phishguardcore}
Aariya N., A. Subashree, Kishore S., Harshini R. L., N. K. Srinithi, and Sana Parveen A., ``PhishGuardCore: An advanced framework for mitigating phishing attacks using hybrid detection techniques,'' in \textit{Proc. 5th Int. Conf. Emerging Research in Electronics, Computer Science and Technology (ICERECT)}, 2025, doi: 10.1109/ICERECT65215.2025.11376232.

\bibitem{kumar2025phishing}
G. Kumar K., V. Devi S., V. T., P. B., and K. N. V., ``Phishing website detection using a hybrid CNN-CapsNet-XGBoost-Random Forest model for cross-platform security,'' in \textit{Proc. 2nd Asia Pacific Conf. Innovation in Technology (APCIT)}, 2025.
